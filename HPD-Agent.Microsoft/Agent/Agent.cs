...
/// No longer inherits from AIAgent - designs its own API surface.
/// </summary>
public sealed class Agent
{
    ...
    /// <summary>
    /// Runs the agent with no message (streaming), assuming all required instructions are provided to the agent or on the thread.
    /// Returns ExtendedAgentRunResponseUpdate to expose HPD-specific event data.
    /// </summary>
    /// <param name="thread">Thread for conversation state</param>
    /// <param name="options">Optional chat options</param>
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>Streaming agent run response updates (extended with HPD-specific event data)</returns>
    public IAsyncEnumerable<ExtendedAgentRunResponseUpdate> RunStreamingAsync(
        AgentThread? thread = null,
        AgentRunOptions? options = null,
        CancellationToken cancellationToken = default) =>
    ...
    /// <summary>
    /// Runs the agent with a text message from the user (streaming).
    /// Returns ExtendedAgentRunResponseUpdate to expose HPD-specific event data.
    /// </summary>
    ...
    /// <summary>
    /// Runs the agent with a single chat message (streaming).
    /// Returns ExtendedAgentRunResponseUpdate to expose HPD-specific event data.
    /// </summary>
    ...
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>Streaming agent run response updates (extended with HPD-specific event data)</returns>
    public async IAsyncEnumerable<ExtendedAgentRunResponseUpdate> RunStreamingAsync(
        IEnumerable<ChatMessage> messages,
        AgentThread? thread = null,
        AgentRunOptions? options = null,
    ...
    /// <returns>A restored Microsoft ConversationThread instance</returns>
    public AgentThread DeserializeThread(JsonElement serializedThread, JsonSerializerOptions? jsonSerializerOptions = null)
    {
