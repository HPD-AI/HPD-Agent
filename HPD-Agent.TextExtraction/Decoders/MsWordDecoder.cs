...
namespace HPD_Agent.TextExtraction.Decoders
{
    public sealed class MsWordDecoder : IContentDecoder
    {
        ...
        {
            _log = (loggerFactory ?? NullLoggerFactory.Instance).CreateLogger<MsWordDecoder>();
        }

        public bool SupportsMimeType(string mimeType)
        {
            return mimeType != null &&
                   (mimeType.StartsWith(MimeTypes.MsWordX, StringComparison.OrdinalIgnoreCase) ||
                    mimeType.StartsWith(MimeTypes.MsWord, StringComparison.OrdinalIgnoreCase));
        }
        ...
        public Task<FileContent> DecodeAsync(Stream data, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                if (paragraphs != null)
                {
                    foreach (Paragraph p in paragraphs)
                    {
                        ...
                        if (lastRenderedPageBreak != null)
                        {
                            ...
                            sb.Clear();
                            result.Sections.Add(new Chunk(pageContent, pageNumber, Chunk.Meta(sentencesAreComplete: true, pageNumber: pageNumber)));
                            pageNumber++;
                        }
                ...
                result.Sections.Add(new Chunk(lastPageContent, pageNumber, Chunk.Meta(sentencesAreComplete: true, pageNumber: pageNumber)));
