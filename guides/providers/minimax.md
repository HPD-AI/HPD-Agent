...
The MiniMax provider uses provider key `minimax` and the `HPD-Agent.Providers.MiniMax` package. It is backed by HPD's shared OpenAI-compatible chat-completions client, so `ModelName` is a MiniMax chat model id.
...
The default endpoint is `https://api.minimax.io/v1/`, which sends chat requests to `chat/completions`. Pass `endpoint` to `WithMiniMax(...)`, set `Endpoint` in `ClientProviderConfig`, or use `MINIMAX_ENDPOINT / MINIMAX_BASE_URL / MINIMAX_API_BASE` when you need a compatible proxy or custom base URL.
...
var agent = await new AgentBuilder()
    .WithMiniMax(model: "MiniMax-M3")
    .BuildAsync();

var result = await agent.RunAsync("Write one sentence about MiniMax setup.");
...
MiniMax exposes additional thinking and multimodal parameters that are not surfaced as typed HPD options yet. The provider currently registers the common chat path.
