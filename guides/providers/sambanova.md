...
The SambaNova provider uses provider key `sambanova` and the `HPD-Agent.Providers.SambaNova` package. It is backed by HPD's shared OpenAI-compatible chat-completions client, so `ModelName` is a SambaNova chat model id.
...
The default endpoint is `https://api.sambanova.ai/v1/`, which sends chat requests to `chat/completions`. Pass `endpoint` to `WithSambaNova(...)`, set `Endpoint` in `ClientProviderConfig`, or use `SAMBANOVA_ENDPOINT / SAMBANOVA_BASE_URL` when you need a compatible proxy or custom base URL.
...
var result = await agent.RunAsync("Write one sentence about SambaNova setup.");
