...
The Scaleway Generative APIs provider uses provider key `scaleway` and the `HPD-Agent.Providers.Scaleway` package. It is backed by HPD's shared OpenAI-compatible chat-completions client, so `ModelName` is a Scaleway Generative APIs chat model id.
...
The default endpoint is `https://api.scaleway.ai/v1/`, which sends chat requests to `chat/completions`. Pass `endpoint` to `WithScaleway(...)`, set `Endpoint` in `ClientProviderConfig`, or use `SCALEWAY_ENDPOINT / SCALEWAY_BASE_URL / SCW_ENDPOINT / SCW_BASE_URL` when you need a compatible proxy or custom base URL.
...
var agent = await new AgentBuilder()
    .WithScaleway(model: "qwen3.5-397b-a17b")
    .BuildAsync();

var result = await agent.RunAsync("Write one sentence about Scaleway Generative APIs setup.");
...
The default endpoint targets Scaleway serverless Generative APIs. Dedicated deployments use deployment-specific base URLs, so pass `endpoint` for those.
