...
The Venice provider uses provider key `venice` and the `HPD-Agent.Providers.Venice` package. It is backed by HPD's shared OpenAI-compatible chat-completions client, so `ModelName` is a Venice chat model id.
...
The default endpoint is `https://api.venice.ai/api/v1/`, which sends chat requests to `chat/completions`. Pass `endpoint` to `WithVenice(...)`, set `Endpoint` in `ClientProviderConfig`, or use `VENICE_ENDPOINT / VENICE_BASE_URL` when you need a compatible proxy or custom base URL.
...
var result = await agent.RunAsync("Write one sentence about Venice setup.");
