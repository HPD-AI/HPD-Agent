...
The Z.AI provider uses provider key `zai` and the `HPD-Agent.Providers.Zai` package. It is backed by HPD's shared OpenAI-compatible chat-completions client, so `ModelName` is a Z.AI chat model id.
...
The default endpoint is `https://api.z.ai/api/paas/v4/`, which sends chat requests to `chat/completions`. Pass `endpoint` to `WithZai(...)`, set `Endpoint` in `ClientProviderConfig`, or use `ZAI_ENDPOINT / ZAI_BASE_URL / Z_AI_ENDPOINT / Z_AI_BASE_URL / BIGMODEL_ENDPOINT / BIGMODEL_BASE_URL` when you need a compatible proxy or custom base URL.
...
var agent = await new AgentBuilder()
    .WithZai(model: "glm-4.7")
    .BuildAsync();

var result = await agent.RunAsync("Write one sentence about Z.AI setup.");
...
This provider targets Z.AI OpenAI-compatible APIs. `BIGMODEL_API_KEY` is accepted as an alias for older Zhipu/BigModel naming.
