...
/// <summary>
/// Characterization tests for ErrorTrackingIterationMiddleware.
/// These tests document and verify the expected behavior of error tracking.
/// </summary>
public class ErrorTrackingIterationMiddlewareTests
{
    [Fact]
    ...
    public async Task AfterIteration_NoToolResults_DoesNothing()
    {
        ...
        // Assert
        Assert.False(context.Properties.ContainsKey("HasToolErrors"));
        Assert.False(context.Properties.ContainsKey("ShouldIncrementFailures"));
        Assert.False(context.Properties.ContainsKey("ShouldResetFailures"));
    }

    [Fact]
    public async Task AfterIteration_SuccessfulToolResults_SignalsReset()
    {
        // Arrange
        var middleware = new ErrorTrackingIterationMiddleware();
        var context = CreateContext(iteration: 1);
        ...
        // Assert
        Assert.False((bool)context.Properties["HasToolErrors"]);
        Assert.True((bool)context.Properties["ShouldResetFailures"]);
        Assert.False(context.Properties.ContainsKey("ShouldIncrementFailures"));
    }

    [Fact]
    public async Task AfterIteration_ErrorWithException_SignalsIncrement()
    {
        // Arrange
        var middleware = new ErrorTrackingIterationMiddleware();
        ...
        // Assert
        Assert.True((bool)context.Properties["HasToolErrors"]);
        Assert.True((bool)context.Properties["ShouldIncrementFailures"]);
        Assert.False(context.Properties.ContainsKey("ShouldResetFailures"));
    }

    [Fact]
    public async Task AfterIteration_ErrorWithErrorPrefix_SignalsIncrement()
    {
        // Arrange
        var middleware = new ErrorTrackingIterationMiddleware();
        var context = CreateContext(iteration: 1);
    ...
    [Fact]
    public async Task AfterIteration_ErrorWithFailedPrefix_SignalsIncrement()
    {
        // Arrange
        var middleware = new ErrorTrackingIterationMiddleware();
        var context = CreateContext(iteration: 1);
    ...
    [Fact]
    public async Task AfterIteration_RateLimitError_SignalsIncrement()
    {
        // Arrange
        var middleware = new ErrorTrackingIterationMiddleware();
        var context = CreateContext(iteration: 1);
    ...
    public async Task AfterIteration_ErrorExceedsThreshold_TriggersTermination()
    {
        ...
        // State shows 2 consecutive failures, one more will exceed threshold
        var state = CreateStateWithConsecutiveFailures(2);
        var context = CreateContext(iteration: 3, state: state);
        context.ToolResults = new[]
        {
    ...
    public async Task AfterIteration_CustomMessageTemplate_UsesTemplate()
    {
        ...
        Assert.True(context.SkipLLMCall);
        Assert.NotNull(context.Response);
        Assert.Equal("ERRORS DETECTED: 2 out of 2 allowed!", context.Response.Text);
    }
    ...
    private static AgentLoopState CreateStateWithConsecutiveFailures(int count)
    {
        ...
        // Apply WithFailure() count times to simulate consecutive failures
